...
class Model():
  def __init__(self,config):
    ...
    with tf.name_scope('MDN_over_next_vector') as scope:
      ...
      self.mu1,self.mu2,self.mu3,self.s1,self.s2,self.s3,self.rho,self.theta = tf.split(1,params,h_xyz)
      ...
      px1x2x3_mixed = tf.reduce_sum(tf.mul(px1x2x3,self.theta),1)  #Sum along the mixtures in dimension 1
